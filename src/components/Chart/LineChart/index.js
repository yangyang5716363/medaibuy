...
class LineChart extends Component {
  ...
  _renderHtml = (params) => {
    ...
    let ds = `<li><i style="background-color:${color}"></i><span>${seriesName}</span><span style="color:${color}">${v}</span></li>`
    let btn = `<div class="line-tooltip__btn"><button>See Offers</button></div>`
    let str = `<div class="line-tooltip"><p>${name}</p><ul>${ds}</ul>${btn}</div>`
    return str
  }
  // 设置建议信息
  _setOption(props) {
